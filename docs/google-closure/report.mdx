...
28. [REPORTED](https://github.com/google/closure-compiler/issues/4053) some compiler flags are a mess
    ...
    6. --process_common_js_modules should be on by default, unless it is experimental, in which case it should be marked as experimental
    7. module_resolution - what is BROWSER_WITH_TRANSFORMED_PREFIXES? what is the difference between node and webpack. why I must use NODE in order to be able to import from node_modules? or it should be mentioned that i need to do that.
    8.  force_inject_library - base means Google closure base.js? it is not obvious
    9.  source_map_input,source_map_location_mapping - what is the difference between em
    10. third_party - what style rules?
    11. js - not javascript filename but javascript input file(s), explain how glob works, don't just say minimatch. Explain that node_modules have to be included as well
