...
### Array callback methods are not type checked
Array methods such as **filter, map, forEach, reduce, etc.**  are not type-checked. The type of the callback is not inferred! ([reported](https://github.com/google/closure-compiler/issues/4041))

```js
// GCC thinks `item` is `unknown` type!
const positive = [-1,0,1,2,3,4].filter((item) => item > 2);
```
